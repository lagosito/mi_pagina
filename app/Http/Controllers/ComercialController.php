...
class ComercialController extends BaseController
{
	...
	public function getIndex()
	{
		...
		return redirect('comercial/proyectos');
	}
	public function getProyectos()
	{
		$proyectos = DB::table('pry_proyecto')
			->select('*','pry_proyecto.id as idesP')
		...
		return view('comercial.proyectos.proyectos')
	...
	public function postNuevoproyecto(Request $request)
	{
		...
		try {
			...
			DB::commit();
			return redirect('comercial/editar/'.$proyecto->id.'/'.$proyecto->id_cliente);
		}
		catch (\Exception $e) {
	...
	public function getEditarproyecto($id)
	{
		$proyecto = Proyecto::where('id', '=', $id)->first();
		$cliente = Clientes::where('id', '=', $proyecto->id_cliente)->first();
		$user = User::where('id', '=', $cliente->id_usuario)->first();

		return view('comercial.proyectos.editarproyecto')
	...
	public function postObtenerproyecto(Request $request)
	{
		...
		return Funciones::getDatosProyecto($id_proyecto);
	}
	public function postEditproyecto(Request $request)
	{
		...
		$proyecto = Proyecto::find($idproyecto);
		$proyecto->subpagina = $request->input('subpagina');
		$proyecto->fill($request->all());
		$proyecto->save();
	...
	public function getPapelera()
	{
		$proyectos = DB::table('pry_proyecto')
			->join('dat_cliente','dat_cliente.id','=','pry_proyecto.id_cliente')
			->whereNotNull('pry_proyecto.deleted_at')
			->select('pry_proyecto.nombre_proyecto','dat_cliente.cliente','dat_cliente.id','pry_proyecto.id as idesP', 'pry_proyecto.deleted_at',
				'dat_cliente.url_cliente', 'pry_proyecto.url_proyecto')
			->get();

		return view('comercial.proyectos.papelera')
			->with('proyectos',$proyectos);
	}
	public function getRestaurarproyecto($id)
	{
	...
	{
		//elimina el proyecto en base de datos con usuario y contrasenna

		$user = User::find(Auth::user()->id);

		if (Hash::check($request->input('password'), $user->password) && $request->input('username')==Auth::user()->username)
		{
			$id_proyecto= $request->input('id_proyecto');
			...
			try {
				Funciones::deleteProyecto($id_proyecto);
	...
	public function postEliminarsubpagina(Request $request){
		$id_url= $request->input('id');
		//Funciones::eliminarSubPagina($id_url);
	...
	{
		$usuarios = User::whereNotIn('nivel_id', [1])->get();
		$roles = Rol::lists('rol', 'id');
		$admin = 1;

		return view('comercial.usuarios.usuarios')
	...
	{
		$usuarios = User::where('nivel_id', '=', 1)->get();
		$roles = Rol::lists('rol', 'id');
		$admin = 0;

		return view('comercial.usuarios.usuarioscliente')
			->with('usuarios', $usuarios)
			->with('roles', $roles)
			->with('admin', $admin);
	}
	...
	{
		$roles = Rol::lists('rol', 'id')->all();

		return view('comercial.usuarios.nuevousuario')
			->with('roles', $roles);
	}
	public function postNuevousuario(Request $request)
	{
		if($request->input('disponibilidad-usuario')=='1')
		{
			return redirect()->back()->with('mensaje', 'Usuario no disponible');
		}
		$nuevo = new User();
		$nuevo->fill($request->all());
		$nuevo->password = bcrypt($nuevo->password);
		$nuevo->save();

		if($nuevo->nivel_id==1)
		{
			return redirect('comercial/usuarioscliente');
		}
		else
		{
			return redirect('comercial/usuariosadmin');
		}
	...
	public function getEditarusuario($id)
	{
		...
		return view('comercial.usuarios.editarusuario')
	...
	/*public function getPry()
	{
		$prys = DB::table('pry_modulo')
			->where('tipo_id','=','1')
			->first();
	...
	public function postSavemodulocabecera(Request $request)
	{
		...
		//save link logo
		$modulo = PryModulo::find($id_modulo);
		$modulo->link = $request->input('link_logo');
		$modulo->save();
	...
	public function postObtenercabecerasinternas()
	{
		$data = Funciones::getCabecerasInternasHtml();
		return $data;
	}
	...
	public function getModulos()
	{
		$modulos = Modulo::get();
		$agencias = Agencia::lists('nombre_agencia', 'id')->all();
		$clientes = Clientes::select('id', 'id_agencia', 'razon_social')->get();
		$categorias= ModuloCategoria::lists('categoria', 'id')->all();

		return view('comercial.modulos')
				->with('modulos', $modulos)
				->with('agencias', $agencias)
				->with('clientes', $clientes)
				->with('categorias', $categorias);
	}
	public function postModlsun(Request $request)
	{
		$id_modulo = $request->input('idmodls');
		$id_agencia = $request->input('id_agencia');

		$modulo = Modulo::find($id_modulo);
		$modulo->id_md_categoria = $request->input('id_categoria');
		$modulo->titulo=$request->input('nmodulo');
		$modulo->comentario=$request->input('modldesc');
		$modulo->save();

		$id_clientes = $request->input('id_clientes');

		$total_clientes = count($id_clientes);

		ClienteModulo::where('id_md_modulo','=', $id_modulo)->delete();

		for($i=0; $i<$total_clientes; $i++)
		{
			$item = new ClienteModulo();
			$item->id_md_modulo = $id_modulo;
			$item->id_agencia = $id_agencia;
			$item->id_cliente = $id_clientes[$i];
			$item->save();
		}

	}
	public function postEliminarimgcontenido(Request $request)
	{
		$id = $request->input('id_modulo_estructura');
		$id_proyecto = $request->input('id_proyecto');

		Funciones::eliminarImgcontenido($id, $id_proyecto);
	}
	public function getAgencias()
	{
		...
		return view('comercial.usuarios.agencias')
			->with('agencias', $agencias)
			->with('usuarios', $usuarios);
	}
	public function getEditaragencia($id)
	{
		$agencia = Agencia::where('id', '=', $id)->first();
		$usuarios = User::lists('name', 'id')->all();

		return view('comercial.usuarios.editaragencia')
			->with('agencia', $agencia)
			->with('usuarios', $usuarios);
	}
	public function postEditaragencia($id, Request $request)
	{
		$agencia = Agencia::find($id);
		$agencia->fill($request->all());
		$agencia->save();

		return redirect('comercial/agencias');
	}

	public function getNuevaagencia()
	{
		$usuarios = User::lists('name', 'id')->all();

		return view('comercial.usuarios.nuevaagencia')
			->with('usuarios', $usuarios);
	}
	public function postNuevaagencia(Request $request)
	{
		$nuevo = new Agencia();
		$nuevo->fill($request->all());
		$nuevo->save();
	...
	public function postClientesxagencia(Request $request)
	{
		$id_agencia  = $request->input('id_agencia');
		$data = Funciones::getClientesporAgencia($id_agencia);

		return $data;
	}
	public function postAgenciamodulo(Request $request)
	{
		$id_modulo  = $request->input('id_modulo');

		$agencia = ClienteModulo::where('id_md_modulo', '=', $id_modulo)->select('id_agencia')->first();

		if(isset($agencia))
		{
			return $agencia->id_agencia;
		}
		else
		{
			return '0';
	...
	public function postClientesexclusivosmodulo(Request $request)
	{
		$id_agencia  = $request->input('id_agencia');
		$id_modulo = $request->input('id_modulo');
		...
		if($id_agencia!='0')
		{
			$data = Funciones::getClientesexclusivosmodulo($id_agencia, $id_modulo);
		}
	...
	public function getCategoriasmodulos()
	{
		...
		return view('comercial.modulos.categorias')
			->with('categorias', $categorias);
	}
	public function postNuevomdcategoria(Request $request)
	{
	...
	public function postEditarmdcategoria(Request $request)
	{
		$categoria = ModuloCategoria::find($request->input('id_categoria'));
		$categoria->categoria = $request->input('categoria');
		$categoria->save();
	...
	{
		$prueba = Tinify::setKey("Jsn6qaRf7CQate13FO-I_l6sA2bS5BtN");
		$source = Tinify::fromFile("upload/1/galeria/0909_57d31beac3c49.png");
		$source->toFile("upload/pruebas/optimized.png");
	}*/
	/*public function getCarpetas(){
		if ($handle = opendir('upload/6/slider/')) {
		    $blacklist = array('.', '..', 'somedir', 'somefile.php');
		    while (false !== ($file = readdir($handle))) {
		        if (!in_array($file, $blacklist)) {
		            echo $file."<br/>";
		            $prueba = Tinify::setKey("Jsn6qaRf7CQate13FO-I_l6sA2bS5BtN");
					$source = Tinify::fromFile('upload/6/slider/'.$file);
					$source->toFile("upload/6/slider/".$file);
		        }
		    }
		    closedir($handle);
		    echo 'end';
