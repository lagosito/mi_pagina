<link href="<?php echo url::asset('/css/modulo_c8_tabs.css'); ?>" rel="stylesheet" type="text/css" />
...
@endif
<style>
    #container-modulo_c8_tabs .titulo {
        font-size: <?php echo 60+ $proyecto->font_size?>px;
    }
    #container-modulo_c8_tabs .nav-pills>li>a {
        font-size: <?php echo 14+ $proyecto->font_size?>px;
    }
    #container-modulo_c8_tabs .item-nombre {
        font-size: <?php echo 15+ $proyecto->font_size?>px;
    }

    #container-modulo_c8_tabs .linkareferencia{
        font-size: <?php echo 10+ $proyecto->font_size?>px;
    }
    #container-modulo_c8_tabs .texto {
        font-size: <?php echo 13+ $proyecto->font_size?>px;
    }
    @media (min-width: 768px) {
        #container-modulo_c8_tabs .titulo-formgaleria {
...
<div id="container-modulo_c8_tabs" class="wrapper outer">
    ...
    <div class="bloquecontabs">
        <div class="text-center">
            <ul class="pestanas nav nav-pills center-pills">
                @if($editar==0)
                    <li class="active tab-pill"><a data-toggle="pill" href="#tabcattodos">Todos</a></li>
                    @foreach($tabs as $tab)
        ...
        <div class="tab-content">
            @if($editar==0)
                <div id="tabcattodos" class="tab-pane fade in active">
                    <?php
                    $i=0;
                    $galeriaItems = App\Console\Commands\Funciones::getAllGaleriaItems($pry_modulo->idesPR, $idioma); 
                    $tam_todos = count($galeriaItems);
                    $item_activo=0;?>
                    <div class="row items-todos" style="margin-bottom: 20px">

                        <div id="slide_md_c8_{{$pry_modulo->idesPR}}tabs0" class="carousel slide slide_md_c8_tabs" >

                            <!-- Wrapper for slides -->
                            <div class="carousel-inner carousel-inner-c8-tabs" id="carousel-inner-c8-{{$pry_modulo->idesPR}}tabs0">
                                @if($item_activo==0)
                                    <div class="item active">
                                    <div class="box-flex">
                                    <?php $item_activo=1; $i=1;?>
                                @endif
                            ...
                            @if($tam_todos >8)
                                ...
                                <a class="left carousel-control" href="#slide_md_c8_{{$pry_modulo->idesPR}}tabs0" role="button" data-slide="prev">
                                <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
                                <span class="sr-only">Previous</span>
                                </a>
                                <a class="right carousel-control" href="#slide_md_c8_{{$pry_modulo->idesPR}}tabs0" role="button" data-slide="next">
                ...
                <?php
                    ...
                    <div class="row items-galeria{{$tab->id}}" style="margin-bottom: 20px">
                        ...
                        <div id="slide_md_c8_tabs{{$tab->id}}" class="carousel slide slide_md_c8_tabs" >
                            ...
                            <!-- Wrapper for slides -->
                            <div class="carousel-inner carousel-inner-c8-tabs" id="carousel-inner-c8-tabs{{$tab->id}}">
                                @if($item_activo==0)
                                    <div class="item active">
                                    <div class="box-flex">
                                    <?php $is_activo=1; $i=1;?>
            ...
            <div class="botonlink">
             @if($editar==0)
                ...
                <a class="linkareferencia fondo-boton" href="{{$modulosDtll[0]->url}}" target="_blank">{!!$modulo->subtitulo!!}</a>
                @elseif($modulosDtll[0]->es_url_externo==0)
                    <a class="ancla linkareferencia fondo-boton" href="#{{str_replace(' ', '', $modulosDtll[0]->url)}}">{!!$modulo->subtitulo!!}</a>
                @else
                    <a class="linkareferencia download fondo-boton" id="linkareferencia{{$modulosDtll[0]->id}}" href="{!!url('upload/'.$pry_modulo->id_proyecto.'/archivos/'.$modulosDtll[0]->url)!!}"  download="{{$modulo->titulo}}">{!!$modulo->subtitulo!!}</a>
                @endif
            @else
                <a class="linkareferencia edit-linkareferencia fondo-boton" id="linkareferencia{{$modulosDtll[0]->id}}" href="{{$modulosDtll[0]->url}}"  data-pk="{{$modulosDtll[0]->id}}" data-url="{{$modulosDtll[0]->url}}" data-texto="{{$modulosDtll[0]->subtitulo}}" data-url-externo="{{$modulosDtll[0]->es_url_externo}}" data-name="subtitulo" data-toggle="popover" data-trigger="click" data-popover-content="#popoverlinkareferencia" title="">{!!$modulo->subtitulo!!}</a>
            @endif
...
@if($editar==0)
    ...
    <script>
        ...
        $(document).on('click', '.tab-pill', function(){
            ...
            console.log(content);
            $(content).find('.carousel-inner-c8-tabs .item').
            ();
        });
        ...
        function createCrewItem(tab, nombre, texto, id_proyecto, detalle1, detalle2)
        {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    /*$('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.editar-crew-item').attr('data-item', nombre);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.editar-crew-item').attr('data-texto', texto);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.editar-crew-item').attr('data-id', data.id);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.editar-crew-item').attr('data-imagen', 'image_not_available.png');
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.item-nombre').text(nombre);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.texto').text(texto);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('img').attr('src', ruta+'/image_not_available.png' );
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.editar-crew-item').text('Editar');
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.eliminar-crew-item').removeClass('nomostrar2');
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.eliminar-crew-item').attr('data-id', data.id);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().parent().attr('id', 'div_itemcrew'+ data.id);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().parent().attr('data-id', data.id);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().parent().attr('data-tab', data.id_pry_galeria_tab);
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).parent().find('.portlet-header').removeClass('nomostrar');
                    $('#crewitem0-tab'+data.id_pry_galeria_tab).attr('id', 'crewitem'+ data.id);


                    $(".newCrewitem" ).removeClass('newCrewitem');

                    $(nuevo).appendTo('.items-galeria'+data.id_pry_galeria_tab);
                    $(nuevo).find('.editar-crew-item').attr('data-tab', data.id_pry_galeria_tab);
                    $(nuevo).find('img').attr('id', 'crewitem0-tab'+data.id_pry_galeria_tab);
                    $(nuevo).removeClass('clonarCrewitem');
